...
app.get("/", (c) => {
    ...
    const body = {
        ...
        name: agentName,
        description: process.env.AGENT_DESCRIPTION ?? "A2A agent for chatting with Web3 Agent",
        image: imageUrl,
        endpoints: [
            {
                name: "A2A",
...
app.get("/.well-known/agent-card.json", (c) => {
    ...
    const body = {
        type: "https://eips.ethereum.org/EIPS/eip-8004#registration-v1",
        name: agentName,
        description: process.env.AGENT_DESCRIPTION ?? "General-purpose agent registered via ERC-8004 demo.",
        image: imageUrl,
        endpoints: [
            {
                name: "A2A",
